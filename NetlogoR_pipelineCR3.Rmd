...
data <- read.table("bs_CRnometa_9.csv",
...
test <- data %>% 
  select(run_num = X.run.number.,                  # each run number is an experimental unit
         ...
         instant_growth = X.instantaneous.growth..of.fishes)
...
test <- test %>%
  ...
  separate(key, into = c("msg", "fish_id"), sep = "_") %>% 
  spread(msg, value) %>% 
...
test <- test %>% 
  ...
  separate(growth, 
           ...
           sep = " ") %>%
  select(-c(growth_0, size_0)) # I drop this because there's only a value for ind 1
test[,-c(1:4)] <- round(as.numeric(unlist(test[,-c(1:4)])), 3)
...
test <- test %>%
  ...
  separate(key, into = c("sg", "time"), sep = "_") %>% 
  spread(sg, value) %>% 
...
ind_time <- test %>%
  select(run_num, density, sync, asym, synchrony, asymmetry,
         time, n_surv, n_dead, biomass, fish_id, meta, size, growth) %>% 
  rename(id = fish_id)
