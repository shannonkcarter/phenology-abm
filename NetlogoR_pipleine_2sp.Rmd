...
ind <- ind_time %>% 
  group_by(run_num, sp, id, meta, sync_fish, sync_dfly, mean_dfly, surv_dfly, surv_fish) %>%
...
check <- ind %>% 
  group_by(run_num, sp, surv_dfly, surv_fish) %>%
  summarize(meta_check = sum(meta, na.rm = T))
