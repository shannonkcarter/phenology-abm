...
data <- read.table("bs_CRnometa_12.csv",
...
run <- run %>% 
  ...
  gather(survival, biomass, size, emergence_time, key = "variable", value = "abs_value") %>% 
  arrange(run_num)

# now by treatment
trt <- run %>% 
  group_by(sync, synchrony, asym, asymmetry, variable) %>% 
  summarise(mean = mean(abs_value))
...
baseline_means <- trt %>% 
  subset(synchrony == 'med' & asymmetry == 'symm') %>% 
  dplyr::select(variable = variable, baseline_value = mean) #%>% 
  #dplyr::select(variable, baseline_value)
...
trt <- trt %>% 
  mutate(baseline_adj = (mean - baseline_value)/baseline_value)
...
#set.seed(123)
trt_nobaseline <- subset(trt, subset = asymmetry.x != "symm" & synchrony.x != "med")
run_nobaseline <- subset(run, subset = asymmetry.x != "symm" & synchrony.x != "med")

ggplot(trt_nobaseline, 
       aes(x = variable, y = baseline_adj, color = synchrony.x, shape = asymmetry.x,
           group = interaction(asymmetry.x, synchrony.x))) +
  geom_jitter(data = run_nobaseline,
              aes(x = variable, y = baseline_adj, color = synchrony.x, shape = asymmetry.x),
              size = 2, alpha = 0.25, width = 0.2) +
  #geom_segment(aes(x = variable, xend = variable, y = 0, yend = baseline_adj),
  #            size = 0.8, 
  ...
  #                   labels = c("low_asym, low", "low_asym, high", "high_asym, low", "high_asym, high"),
  #                   values = c(19, 19, 17, 17)) +
  labs(x = NULL, y = "Deviation from baseline case\n(medium synchrony, symmetric competition)") +
