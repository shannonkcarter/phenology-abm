...
levels(params$sync_dfly) <- c("high", "low")
levels(params$mean_dfly) <- c("same", "fish early")
...
Looks like in 'fish early' scenarios, there are some fish hatching on the last day. Look into this.
```{r}
...
plot5 <- ggplot(trt, aes(x = mean_dfly, y = mean_surv_dfly, color = sync_dfly)) + mytheme +
  geom_point(size = 2) + 
  geom_errorbar(width = 0, aes(max = mean_surv_dfly + se_surv_dfly, min = mean_surv_dfly - se_surv_dfly)) +
  stat_smooth(size = 4, method = "lm", se = T, aes(x = mean_dfly, fill = sync_dfly)) +
