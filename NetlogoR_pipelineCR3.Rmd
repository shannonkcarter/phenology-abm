...
params <- read.csv("params_1sp.csv", header = T)
...
run$asymmetry <- factor(run$asymmetry, levels = c("none", "weak", "strong"))
...
data <- read.table("bs_CRnometa_23.csv",
                   header = T,  
...
write.csv(params, "params_1sp.csv")
...
ggplot(trt, aes(x = synchrony, y = abs_value_exp, 
  ...
  geom_errorbar(aes(ymin = (abs_value_exp - se_exp), ymax = abs_value_exp + se_exp), width = 0) +
...
# NEW: also calculate log response ratio-- better for representing small baseline values
meandiffs_run <- run_dabest %>% 
  mutate(umd = (abs_value - baseline_mean)/baseline_mean,
         lrr = log(abs_value/baseline_mean))
...
meandiffs_trt <- trt_dabest %>% 
  mutate(umd = (abs_value_exp - baseline_mean)/baseline_mean,
         ...
         lrr = log(abs_value_exp/baseline_mean))
...
tiff("fig2.tiff", height = 7500, width = 5400, units = "px", res = 800, compression = "lzw")
plot(fig2)
dev.off()
```
...
meandiffs_pres <- ggplot(subset(meandiffs_trt, subset = (var != "emer")),
                     aes(x = as.factor(synchrony), y = umd, 
                          color = asymmetry, shape = asymmetry, fill = asymmetry, group = asymmetry)) + pres_theme +
  geom_smooth(span = 5) +
  ...
  scale_x_discrete(breaks = c("0", "15" ,"30"), labels = c("high", "med", "low"))
meandiffs_pres
ggsave("mean_diffs1sp.tiff")


ind_fewsync <- subset(ind, subset = synchrony == c(3, 15, 27))
ind_fewsync$synchrony1 <- as.character(ind_fewsync$synchrony)
ind_fewsync$synchrony1 <- fct_recode(ind_fewsync$synchrony1, "high synchrony" = "3", "medium synchrony" = "15", "low synchrony" = "27")

ind_fewsync$synchrony1 <- factor(ind_fewsync$synchrony1, levels = c("high synchrony", "medium synchrony", "low synchrony"))

ind_fewsync$asymmetry1 <- fct_recode(ind_fewsync$asymmetry, "competitive symmetry" = "none", "weak asymmetry" = "weak", "strong asymmetry" = "strong")

ind_fewsync$outcome <- as.character(ind_fewsync$meta)
ind_fewsync$outcome <- fct_recode(ind_fewsync$outcome, "survived" = "1", "died" = "0")


# growth rate by hatch date
ind_grow <- ggplot(ind_fewsync, aes(x = hatch_date, y = growth_rate, color = outcome)) +
  geom_point(size = 1.5, alpha = 0.8, position = 'jitter') +
  geom_boxplot(data = ind_fewsync, mapping = aes(y = growth_rate, x = 165, color = outcome, fill = outcome),
               width = 30, alpha = 0.5, size = 1.2) +
  facet_grid(asymmetry1 ~ synchrony1) + mytheme +
  scale_x_continuous(limits = c(0, 180), breaks = seq(0, 120, by = 30)) +
  ylim(0,0.25) +
  geom_vline(aes(xintercept = 140), linetype = "dashed") +
  labs(x = "arrival date",
       y = "growth rate (final size / life span)") + pres_theme +
  scale_color_manual(values = c("black", "#5DBCD2")) +
  scale_fill_manual(values = c("black", "#5DBCD2")) 

ind_grow
ggsave("ind_grow.tiff")
```
...
# build absolute data plot
rawdata <- ggplot(data = ind_fewsync, aes(x = synchrony1, y = abs_value, 
                fill = asymmetry, color = asymmetry, group = asymmetry, shape = asymmetry)) + 
  mytheme +
  geom_point(size = 1.5, alpha = 1/2,
             position = position_dodge(width = 0.5)) +
...
dead_avgs <- ind %>% 
  group_by(run_num) %>%
  # only survivors get factored into these metrics-- we don't care how long it took someone to die
  filter(meta == 0) %>%                                # response variables only consider survivors
  summarise(mean_mass = mean(final_size),              # should be biom/surv
            #se_mass   = sd(final_size)/sqrt(n_dead),# error
            mean_emer = mean(end_date),                # emer is emergence date-- phenology of emergence
            #se_emer   = sd(end_date)/sqrt(n_dead),  # error
            mean_grow = mean(growth_rate)) %>%              # grow is individual time to development, maxsize/time
            #se_grow   = sd(growth_rate)/sqrt(n_dead)) %>% 
  gather(mean_mass, mean_emer,       # gather all response variables
         mean_grow,  
         key = "variable", value = "abs_value") %>% 
...
dead_avgs <- left_join(dead_avgs, params, by = c("run_num"))
dead_trt <- dead_avgs %>% 
  group_by(synchrony, asymmetry, var) %>% 
  summarise(abs_value_exp = mean(mean),
            se_exp    = sd(mean)/sqrt(6))
dead_trt <- dead_trt[is.finite(dead_trt$abs_value_exp),]

variable_names <- list(
  'mass' = "per capita mass",
  'grow' = "per capita growth", 
  'emer' = "advancement date"
)
...
ded <- ggplot(dead_trt, aes(x = synchrony, y = abs_value_exp, 
                            color = asymmetry, fill = asymmetry, shape = asymmetry)) +
  geom_smooth(method = "lm", formula = y ~ splines::bs(x, 3)) +
  geom_point(size = 2.5, color = "black") + mytheme +
  geom_errorbar(width = 0, aes(ymax = abs_value_exp + se_exp, ymin = abs_value_exp - se_exp)) +
  facet_wrap(~var, scales = 'free', labeller = variable_labeller) +
  scale_color_manual(values = c("#acbab3",  "#fbd364", "#446353")) +
  scale_fill_manual(values = c("#acbab3",  "#fbd364", "#446353")) +
  scale_shape_manual(values = c(21, 24, 23)) +
  labs(x = "phenological synchrony", y = "measurement")
ded
#ggsave("dead_avgs.tiff")  
```
...
ind_fewsync <- subset(ind, subset = synchrony == c(3, 15, 27))
ind_fewsync$synchrony1 <- as.character(ind_fewsync$synchrony)
ind_fewsync$synchrony1 <- fct_recode(ind_fewsync$synchrony1, "high synchrony (σ = 3)" = "3", "medium synchrony (σ = 15)" = "15", "low synchrony (σ = 27)" = "27")

ind_fewsync$synchrony1 <- factor(ind_fewsync$synchrony1, levels = c("high synchrony (σ = 3)", "medium synchrony (σ = 15)", "low synchrony (σ = 27)"))
...
# growth rate by hatch date
fig4 <- ggplot(ind_fewsync, aes(x = hatch_date, y = growth_rate, color = outcome)) +
  geom_point(size = 1.5, alpha = 0.8, position = 'jitter') +
  #stat_smooth(se = F) +
  ...
  geom_vline(aes(xintercept = 140), linetype = "dashed") +
  labs(x = "arrival date",
       y = "growth rate (final size / life span)") + mytheme +
# theme_bw(base_size = 15, base_family = "Franklin Gothic Medium") +
#   theme(legend.text = element_text(size = 10),
#         legend.title = element_text(size = 11),
#         text = element_text(size = 14),     
#         axis.title = element_text(size = 12),
...
fig4
tiff("fig4.tiff", height = 5000, width = 7000, units = "px", res = 800, compression = "lzw")
plot(fig4)
dev.off()
...
```{r, echo = F}
ind_fewsync <- subset(ind, subset = (synchrony == c(9, 21)))
ind_fewsync$synchrony <- as.factor(ind_fewsync$synchrony)
ggplot(ind_fewsync, aes(x = hatch_date,  fill = synchrony)) + mytheme +
  geom_density(alpha = 0.75, size = 1) +
  #facet_grid(. ~ synchrony) +
  scale_x_continuous(limits = c(0, 120), breaks = seq(0, 120, by = 20)) +
  labs(x = "arrival date",
       y = "density of individuals") +
  #scale_colour_manual(values = c("#f03b20", "#feb24c")) +
...
ind$synchrony <- as.factor(ind$synchrony)
ggplot(ind, aes(x = id, y = biomass, color = asymmetry)) + mytheme +
  geom_line(size = 2) +
  #facet_grid(. ~ synchrony) +
  #scale_x_continuous(limits = c(0, 120), breaks = seq(0, 120, by = 20)) +
  labs(x = "size, s",
