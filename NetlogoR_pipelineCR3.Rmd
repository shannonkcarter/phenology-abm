...
run <- ind %>% 
  ...
  summarise(surv = sum(meta),
            mean_size = mean(meta * max_size),
            mean_emer = mean(meta * end_date))
...
run <- run %>% 
  ...
  gather(surv, biomass, mean_size, mean_emer, key = "variable", value = "abs_value") %>% 
  arrange(run_num)
...
ggplot(trt_nobaseline, 
  ...
  geom_point(aes(variable, baseline_adj), 
             ...
             position = position_dodge(width = 0.5)) +
  coord_flip() +
  scale_y_continuous(limits = c(-0.25, 0.5), expand = c(0.005, 0.005)) +
