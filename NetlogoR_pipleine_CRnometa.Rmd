...
test <- read.table("bs_CRnometa_4.csv",
                   header = T,  
...
test <- test %>% 
  group_by(run_num) %>% 
  gather(as.character(1:total_time), key = "time", value = 'current_size_fish') %>%
...
ind_time <- test %>%
  select(run_num, n_fish, sync, asym, synchrony, asymmetry,
...
ind <- ind_time %>% 
  group_by(run_num, id, sync, asym, synchrony, asymmetry) %>%
  summarise(max_size = max(size),
            meta = if_else(max_size >= 8, 1, 0),
...
# first, by run number
run <- ind %>% 
  group_by(run_num, sync, synchrony, asym, asymmetry) %>%
  filter(meta == 1) %>% 
