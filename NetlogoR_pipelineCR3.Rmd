...
test <- data %>% 
  select(run_num = X.run.number.,                  # each run number is an experimental unit
         ...
         meta = X.meta...of.fishes,
         sizes_fish = X.fish.size.list..of.fishes, # list of size for each ind for each time
...
test <- test %>% 
  separate(sizes_fish,                                      # separate sizes_fish
           into = paste("size_", c(1:max(test$n_fish)), sep = ""),
           sep = "]") %>%                # every ] marks a new ind
  separate(instant_growth,
           into = paste("growth_", c(1:max(test$n_fish)), sep =  ""),
           sep = "] ") %>% 
  separate(meta,
           into = paste("meta_", c(1:max(test$n_fish)), sep = ""),
           sep = " ")
...
test <- test %>%
  select(run_num, total_time, meta_1: meta_80, size_1:size_80, growth_1:growth_80) %>% 
  gather(key, value, -run_num, -total_time) %>%  
  separate(key, into = c("msg", "fish_id"), sep = "_") %>% 
  spread(msg, value) %>% 
  arrange(run_num, fish_id) %>% 
  select(1:3, meta, everything())
...
test <- test %>% 
  separate(size, 
           ...
           into = paste("size_", c(0:max(test$total_time)), sep =  ""),
           sep = " ") %>% 
  separate(growth, 
           into = paste("growth_", c(0:max(test$total_time)), sep =  ""),
           sep = " ") %>%
...
test <- test %>%
  ...
  gather(key, value, -run_num, -total_time, -fish_id, -meta) %>%  
...
ind_time <- test %>%
  select(run_num,total_time.x, n_fish, sync, asym, synchrony, asymmetry,
         time, survival, n_dead, biomass, fish_id, meta, size, growth) %>% 
  rename(id = fish_id, total_time = total_time.x)
...
ind <- ind_time %>% 
  group_by(run_num, id, sync, asym, synchrony, asymmetry, survival, n_dead, biomass, meta) %>%
...
ind$surv_check <- 80 - (ind$survival + ind$n_dead)
# now by treatment
trt <- run %>% 
...
check <- ind %>% 
  ...
  summarise(no_meta = sum(ind$meta == "yes"))
