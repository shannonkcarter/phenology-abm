...
test <- read.table("bs_CRnometa_7.csv",
...
#df <- data.frame(x = c(NA, "a-A", "a-d", "b-c", "d-e"))
#View(df)
#df %>% extract(x, "A")
```
...
test <- test %>% 
  separate(sizes_fish,                                      # separate sizes_fish
           into = paste("s_", c(1:max(test$n_fish)), sep = ""),
           sep = "]") %>%                # every ] marks a new ind
  separate(instant_growth,
           into = paste("g_", c(1:max(test$n_fish)), sep =  ""),
           sep = "] ")


#test <- test %>% 
#  separate(instant_growth,                                      # separate sizes_fish
#           into = paste("growth_f_", c(1:max(test$n_fish)), sep = ""),
...
test <- test %>%
  select(run_num, total_time, f_1:f_80) %>% 
  unique(.) %>% 
  gather(f_1:f_80, key = "fish_id", value = 'size_fish') %>% 
  arrange(run_num)
