...
# First, separate so that each individual has it's 2 columns: 1 for meta, 1 for size list
test <- test %>% 
...
# Next, I'll gather so individuals appear in rows and spread so column are diff variables
test <- test %>%
...
test <- test %>% 
  separate(s, into = as.character(c(0:250)), sep = " ") %>% 
  select(-c(as.character(0))) %>%         # I drop this because there's only a value for ind 1
  group_by(run_num) %>% 
  gather(as.character(1:250), key = "time", value = 'current_size') %>%
  arrange(run_num, sp, as.numeric(id))

test$current_size <- round(as.numeric(test$current_size), 3)
...
### ADD NEW PARAM VARIABLES TO SELECT
ind_time <- left_join(test, params, by = "run_num")
ind_time <- ind_time %>%
  select(run_num, sync_fish, sync_dfly, mean_fish, mean_dfly,
         asym_fish, asym_dfly, surv_fish, surv_dfly, time, sp,
         id, m, current_size) %>% 
  rename(meta = m)
...
ind <- ind_time %>% 
  group_by(run_num, sp, id, meta, sync_fish, sync_dfly, asym_fish, asym_dfly,
           mean_dfly, surv_dfly, surv_fish) %>%
  summarise(max_size = max(current_size),
            hatch_date = min(time[current_size > 0]),
            end_date = min(time[current_size == max(current_size)]),
            growth_time = end_date - hatch_date,
            growth_rate = max(current_size)/growth_time)
