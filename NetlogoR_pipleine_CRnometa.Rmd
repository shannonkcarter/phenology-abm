...
ind_time <- test %>%
  select(run_num,total_time.x, n_fish, sync, asym, synchrony, asymmetry,
         time, surv_fish, biomass, fish_id, current_size_fish) %>% 
...
ind <- ind_time %>% 
  group_by(run_num, id, sync, asym, synchrony, asymmetry, survival, biomass) %>%
...
# first, by run number
run <- ind %>% 
  group_by(run_num, sync, synchrony, asym, asymmetry, survival, biomass) %>%
  #filter(meta == 1) %>% 
