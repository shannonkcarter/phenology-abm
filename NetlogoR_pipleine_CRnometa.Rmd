...
```{r}
# treatment means
surv_means <- ggplot(trt, aes(x = asymmetry, y = mean_surv_fish, group = sync_fish, 
                              fill = sync_fish, color = sync_fish, shape = sync_fish)) +   
  ...
  labs(shape = "synchrony", 
       fill = 'synchrony', 
       color = "synchrony",
       x = "size-dependent per capita effects",
...
size_means <- ggplot(trt, aes(x = asymmetry, y = mean_size, group = sync_fish, 
                              fill = sync_fish, color = sync_fish, shape = sync_fish)) +   
  geom_line(size = 0.7, position = position_dodge(width = 0.2)) +
  #geom_errorbar(size = 1, width = 0, position = position_dodge(width = 0.2),                   
  #              aes(ymin = mean_surv_fish - se_surv_fish, 
