...
test <- test %>% 
  separate(sizes_fish,                                      # separate sizes_fish
           into = paste("f_", c(1:test$n_fish), sep = ""),  # levels for new var
...
ind <- ind_time %>% 
  group_by(run_num, id, sync_fish, surv_fish) %>%
  summarise(max_size = min(max(size), 11),
            ...
            end_date = ifelse(meta == 1, min(time[size == 20]), min(time[size == -1])),
...
```{r}
ggplot(subset(ind_time, subset = (run_num == 1 & id == "f_10")), 
