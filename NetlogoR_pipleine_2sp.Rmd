...
data <- read.table("ABM_2Consumer_3.csv",
                   header = T,  
...
ggplot(subset(trt, subset = (sp == "f")),
       aes(x = mean_fish, y = mean, color = sync_fish, group = sync_fish)) + mytheme +
  geom_point(size = 2) + 
  ...
  labs(title = "Responses of vital rates to phenology manipulations",
       x = "mean hatching time (relative to competitor)",
       y = "mean +/- se")
...
ggplot(ind_surv, aes(hatch_date)) + mytheme +
  ...
  geom_density(data = ind_surv, size = 1, alpha = 0.5, color = "steelblue4", fill = "steelblue4",
               aes(x = end_date)) +
  facet_grid(sync_fish ~ mean_fish , scales = "free_y") + 
  theme(strip.text.x = element_text(size = 14),
        strip.text.y = element_text(size = 14)) +
  labs(title = "Phenology across ontogeny",
       subtitle = "hatching phenology in gray, emergence phenology in blue",
       x = "date",
       y = "density of individuals")
