...
run <- ind %>% 
  ...
  summarise(mean_emer = sum(meta * end_date))
...
ggplot(trt_nobaseline, 
  ...
  scale_x_discrete(breaks = c("n_surv", "mean_size", "mean_emer", "biomass"),
...
```{r, echo = F}
ggplot(ind, aes(x = final_size, fill = as.factor(meta))) + theme_bw() +
...
ggplot(subset(ind, subset = (meta == 1)), aes(x = end_date, fill = asymmetry)) + theme_bw() +
  geom_density(alpha = 0.5, size = 0.8) +
  facet_grid(. ~ synchrony) +
  scale_fill_uchicago() +
  #geom_vline(xintercept = c(8, 10, 12), linetype = 'dashed') + 
