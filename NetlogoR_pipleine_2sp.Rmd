...
run <- ind %>% 
  group_by(run_num, sp) %>%
  filter(meta == 1) %>% 
  summarise(mean_mass = mean(max_size),
            se_mass   = sd(max_size)/sum(meta),
            mean_emer = mean(end_date),
            se_emer   = sd(end_date)/sum(meta),
            mean_grow = mean(growth_rate),
            se_grow   = sd(growth_rate)/sum(meta)) %>% 
  gather(mean_mass, se_mass, mean_emer, se_emer, mean_grow, se_grow,
         key = "variable", value = "abs_value") %>% 
  separate(variable, into = c("prop","var"), by = "_") %>% 
  spread(prop, abs_value) %>% 
  arrange(run_num, sp)

# Rejoin with parameters & write csv
run <- left_join(run, params, by = "run_num")
write.csv(run, "run.csv")

# Now by treatment
trt <- run %>% 
  group_by(sync_fish, asym_fish, var) %>% 
...
When revisiting the same behavior space multiple times, skip the data processing and just load the final tidy data.
...
```{r, echo = F}
ggplot(ind, aes(x = hatch_date, color = sp, fill = sp)) + theme_bw() +
...
```{r, echo = F}
ggplot(ind, aes(x = hatch_date, y = max_size, color = sp)) +
  geom_point(size = 1, alpha = 0.2, position = 'jitter') +
  stat_smooth() +
  facet_grid(sync_fish ~ asym_fish, scales = 'free') + theme_bw() +
  scale_color_manual(values = c("#F28335", "#489C92")) +
...
ggplot(ind, aes(x = hatch_date, y = growth_time, color = sp)) + theme_bw() +
...
ggplot(ind, aes(x = hatch_date, y = meta, color = sp)) + theme_bw() +
...
```{r, echo = F}
ggplot(trt, aes(x = sync_fish, y = mean_exp, color = asym_fish)) +
  geom_point(size = 2) + 
