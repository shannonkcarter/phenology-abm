...
```{r load table data}
data <- read.table("ABM_2Consumer_5.csv",
                   header = T,  
...
check <- ind %>% 
  ...
  geom_point(alpha = 1/5) + theme_bw() + mytheme +
...
mean_diffs <- subset(trt, subset = (sync_fish == "low"))
mean_diffs <- mean_diffs %>%
  subset(sp == "f") %>% 
  select(c("sync_fish", "mean_fish", "var", "mean", "se")) 
colnames(mean_diffs)[5] <- "baseline_mean"
