...
mytheme <- theme_bw(base_size = 15, base_family = "Poppins") +
...
ggplot(trt, aes(x = synchrony, y = abs_value_exp, group = asymmetry, 
                              fill = asymmetry, color = asymmetry, shape = asymmetry)) + mytheme +  
  geom_line(size = 0.7, position = position_dodge(width = 0.2)) +
  geom_errorbar(size = 1, width = 0, position = position_dodge(width = 0.2),                   
                aes(ymin = abs_value_exp - se_exp.x,
                    ymax = abs_value_exp + se_exp.x)) +  
  geom_point(size = 4, color = 'black', position = position_dodge(width = 0.2)) +   
  ...
  facet_wrap(. ~ var, scales = 'free_y', labeller = variable_labeller) +
