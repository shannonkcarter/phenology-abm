...
data <- read.table("bs_CRnometa_19.csv",
                   header = T,  
...
baseline_means <- trt %>% 
  subset(synchrony == 1 & asymmetry == 'none') 
...
# function called in plot's facet_wrap to write full label names
variable_labeller <- function(variable, value){
  return(variable_names[value])
}
...
mean_diffs <- subset(trt, subset = (synchrony == 1))
mean_diffs <- mean_diffs[,2:4]
colnames(mean_diffs)[3] <- "baseline_mean"

# now, add those baseline averages to the run data
run_dabest <- run[,1:5]
...
mean_diffs <- ggplot(meandiffs_trt, aes(x = synchrony, y = umd, 
  ...
  theme(legend.position = "none",
        plot.title = element_text(hjust = 0.5),
        plot.margin = margin(20, 20, 20, 20)) +
  labs(x = "phenological synchrony", title = "proportional difference\nfrom high synchrony", y = NULL) 

# put them together. can't neatly put a 
