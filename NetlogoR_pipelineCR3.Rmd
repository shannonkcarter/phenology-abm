...
mean_diffs <- subset(trt, subset = (synchrony == "low"))
mean_diffs <- mean_diffs[,2:4]
...
meandiffs_run <- run_dabest %>% 
  mutate(umd = (abs_value - baseline_mean))
meandiffs_run[meandiffs_run$synchrony == "low", "umd"] <- NA 

meandiffs_trt <- trt_dabest %>% 
  mutate(umd = (abs_value_exp - baseline_mean))
meandiffs_trt[meandiffs_trt$synchrony == "low", "umd"] <- NA 
levels(meandiffs_trt$synchrony) <- c(" ", "med - low", "high - low")


## first, the raw data
rawdata <- ggplot(data = run, aes(x = synchrony, y = abs_value, 
                fill = asymmetry, color = asymmetry, group = asymmetry, shape = asymmetry)) + 
  mytheme +
  #stat_smooth() +
  geom_point(data = run, aes(x = synchrony, y = abs_value, fill = asymmetry), size = 1.5,
             position = position_dodge(width = 0.5)) +
  facet_grid(var~., scales = "free", labeller = variable_labeller) +
  labs(title = "raw data", 
       x = NULL, y = NULL) + 
  scale_color_manual(values = c("#acbab3",  "#fbd364", "#446353")) +
  scale_fill_manual(values = c("#acbab3",  "#fbd364", "#446353")) +
...
mean_diffs <- ggplot(meandiffs_trt, aes(x = synchrony, y = umd, 
                          color = asymmetry, shape = asymmetry, fill = asymmetry, group = asymmetry)) + mytheme +
  geom_errorbar(aes(ymax = umd + se_exp.x, ymin = umd - se_exp.x), width = 0,
  ...
  scale_fill_manual(values = c("#acbab3",  "#fbd364", "#446353")) +
  scale_shape_manual(values = c(21, 24, 23))  + 
  facet_grid(var~., labeller = variable_labeller, scales = "free") +
  theme(legend.position = "none",
        plot.title = element_text(hjust = 0.5),
        plot.margin = margin(20, 20, 20, 20)) +
  labs(x = NULL, title = "mean difference", y = NULL) 

plot_grid(rawdata, mean_diffs, ncol = 2)
```
