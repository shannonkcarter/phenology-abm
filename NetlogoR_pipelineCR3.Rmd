...
rawdata <- ggplot(data = run, aes(x = synchrony, y = abs_value, 
  ...
  geom_point(data = run, aes(x = synchrony, y = abs_value, fill = asymmetry), size = 1.5,
             position = position_dodge(width = 0.5)) +
  facet_wrap(~var, scales = "free", labeller = variable_labeller, nrow = 1) +
  labs(y = "measurement", 
       x = NULL) + 
  scale_color_manual(values = c("#acbab3",  "#fbd364", "#446353")) +
  scale_fill_manual(values = c("#acbab3",  "#fbd364", "#446353")) +
...
mean_diffs <- ggplot(meandiffs_trt, aes(x = synchrony, y = umd, 
                          color = asymmetry, shape = asymmetry, fill = asymmetry, group = asymmetry)) + theme_bw() + #mytheme +
  ...
  scale_fill_manual(values = c("#acbab3",  "#fbd364", "#446353")) +
  scale_shape_manual(values = c(21, 24, 23))  + 
  facet_wrap(~var, nrow = 1, labeller = variable_labeller, scales = "free") +
  theme(legend.position = "none",
        strip.background = element_blank(),
        strip.text.x = element_blank()) +
  labs(x = NULL, y = "mean difference")

plot_grid(rawdata, mean_diffs, ncol = 1)
```
