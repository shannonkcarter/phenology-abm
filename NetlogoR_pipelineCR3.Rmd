...
test <- test %>%
  ...
  separate(key, into = c("sg", "fish_id"), sep = "_") %>% 
  spread(sg, value) %>% 
  arrange(run_num)
...
#test2 <- test1 %>% 
#  group_by(run_num, total_time, fish_id, time) %>% 
#  mutate(size_rev = rev(size))

## got it! except now size is written backwards and growth is forward... can both be backwards so it's easier to flip? or obvi both forwards...
