...
run <- ind %>% 
  group_by(run_num, sp) %>%
  summarise(mean_mass = sum(meta * max_size)/sum(meta),    # only using individuals that survived
            mean_emer = sum(meta * end_date)/sum(meta),    # meta * var is 0 for ind that don't meta
            mean_grow = sum(meta * growth_rate)/sum(meta)) # sum(meta) is number of survivors

# gather different response variables
run <- run %>% 
  group_by(run_num, sp) %>% 
  gather(mean_mass, mean_emer, mean_grow, key = "variable", value = "abs_value") %>% 
  arrange(run_num, sp)
