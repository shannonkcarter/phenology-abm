...
run <- ind %>% 
  ...
  summarise(mean_biom = sum(final_size),               # check this and survival against raw output
            ...
            mean_surv = sum(meta)/80,                     # check this an biom against raw output
...
baseline_means <- trt %>% 
  subset(synchrony == 0 & asymmetry == 'none') 
...
variable_names_rel <- list(
  ...
  'emer' = "emergence date (%Δ high sync)"
)
...
rawdata <- ggplot(data = run, aes(x = as.factor(synchrony), y = abs_value, 
  ...
  theme(legend.position = "none", panel.grid.major = element_blank(),
   ...
   scale_x_discrete(breaks=c("0","15","30"),
