...
# having a hard time dealing with different density levels atm
test <- test %>% 
  separate(sizes_fish,                                      # separate sizes_fish
           into = as.character(c(1:test$n_fish)),  # levels for new var
...
test_fish <- test %>%
  select(run_num, as.character(c(1:test$n_fish))) %>% 
  unique(.) %>% 
  gather(as.character(c(1:test$n_fish)), key = "fish_id", value = 'size_fish') %>% 
  arrange(run_num)
...
```{r}
ggplot(subset(ind_time, subset = (run_num == 1)), 
       aes(x = time, y = size, color = id)) +
  #geom_point() +
